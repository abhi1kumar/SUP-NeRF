...
from data.KITTI.kitti_object_vis.waymo_object import waymo_object, get_lidar_in_image_fov, depth_region_pt3d
...
class WaymoData:
    def __init__(self, hpams,
        ...
        """
            ...
            Object 'Instance' here respects the definition from Waymo dataset.
        ...
        """
        self.waymo_cat = hpams['dataset']['waymo_cat']
        ...
        subset_index_file = 'data/Waymo/waymo.' + split + '.' + self.waymo_cat + '.json'
        if os.path.exists(subset_index_file):
            waymo_subset = json.load(open(subset_index_file))
            if waymo_subset['box_iou_th'] != self.box_iou_th or waymo_subset['max_dist'] != self.max_dist or waymo_subset[
                'mask_pixels'] != self.mask_pixels or waymo_subset['min_lidar_cnt'] != self.min_lidar_cnt or waymo_subset[
                'seg_type'] != self.seg_type or waymo_subset['min_depth'] != self.min_depth:
    ...
    def preprocess_dataset(self, split, subset_file):
        ...
        for data_idx in tqdm(data_ids):
            ...
            lidar_filename = os.path.join(self.waymo.lidar_dir, "%06d.bin" % (int(data_idx)))
            ...
            for obj_idx, obj in enumerate(objects):
                ...
                # ATTENTION: kitti obj location is defined on the ground (waymo the same), but nusc as box center
                corners_3d = corners_of_box(obj_pose, wlh, is_kitti=True)
                pts_ann_indices = pts_in_box_3d(imgfov_pc_rect, corners_3d, keep_top_portion=0.8)
                ...
                is_valid = False
                # assign truncation value since its missing from converted dataset
                pred_uv = view_points(
                    corners_3d,
                    K, normalize=True)
                if pred_uv[0, :].min() < 0 or pred_uv[0, :].max() >= img_width or pred_uv[1, :].min() < 0 or pred_uv[1, :].max() >= img_height:
                    obj.truncation = 1

                # remove lidar_cnt >= self.min_lidar_cnt but add image mean to ignore night
                if tgt_ins_id is not None and tgt_ins_cnt > self.mask_pixels and box_iou > self.box_iou_th and \
                        ...
                        area_ratio < 0.7 and \
                        lidar_cnt >= self.min_lidar_cnt and \
                        T_obj[2] > self.min_depth and img.mean() > 70 and \
                    ...
                    if obj_idx not in self.sample_attr[data_idx].keys():
                        ...
                        # self.sample_attr[data_idx][str(obj_idx)] = {'seg_id': tgt_ins_id}
                    is_valid = True
        ...
        waymo_subset = {}
        waymo_subset['all_valid_samples'] = self.all_valid_samples
        waymo_subset['sample_attr'] = self.sample_attr
        waymo_subset['box_iou_th'] = self.box_iou_th
        waymo_subset['max_dist'] = self.max_dist
        waymo_subset['min_depth'] = self.min_depth
        waymo_subset['mask_pixels'] = self.mask_pixels
        waymo_subset['min_lidar_cnt'] = self.min_lidar_cnt
        waymo_subset['seg_type'] = self.seg_type
        json.dump(waymo_subset, open(subset_file, 'w'), indent=4)
    ...
    def __getitem__(self, idx):
        ...
        # TODO: manual fit wlh to nuscenes tightness
        wlh = np.array([obj.w * 0.9, obj.l * 0.9, obj.h * 1.05])
        corners_3d = corners_of_box(obj_pose, wlh, is_kitti=True)
        ...
        if self.add_pose_err == 1:
            ...
            # adapted to waymo version
            obj_pose_w_err = get_random_pose2(K,
                                              np.asarray(box_2d).astype(np.int32),
                                              yaw_lim=np.pi, angle_lim=self.rand_angle_lim,  # kitti format only has yaw rotation
        ...
        if self.add_pose_err == 3:
            objects_pred = self.waymo.get_pred_objects(int(data_idx))
            asso_obx_id, box_iou = get_associate_box_3d(objects_pred, ins_masks[tgt_ins_id], self.waymo_cat)
        ...
        if self.out_gt_depth or self.debug:
            ...
            # ATTENTION: waymo obj location is defined on the ground, but nusc as box center
        ...
        # check_indices = [60, 125, 185, 217, 220, 260, 282, 309, 320, 326, 327, 346, 547, 553, 555, 594, 601, 633, 651, 662, 671, 720, 740, 780, 812, 829, 843, 845, 851, 863, 869, 879, 894, 958, 1001, 1013, 1033, 1036, 1066, 1080, 1081, 1151, 1152, 1205, 1221]
        # if self.debug and idx in check_indices:
        if self.debug:
            print(
                f'        tgt instance id: {tgt_ins_id}, ')
                # f'lidar pts cnt: {lidar_cnt} ')
...
if __name__ == '__main__':
    ...
    with open('jsonfiles/supnerf.waymo.car.json', 'r') as f:
        hpams = json.load(f)

    waymo_data_dir = hpams['dataset']['data_dir']
    # det3d_path = 'data/third_party_det3D/fcos3d/result_waymo'

    waymo_dataset = WaymoData(
        hpams,
        waymo_data_dir,
        split='val',
        debug=False,
    ...
    dataloader = DataLoader(waymo_dataset, batch_size=1, num_workers=0, shuffle=False)
    ...
    plt.savefig('eval_summary/waymo_dist_hist.pdf')
    plt.close()
    ...
    plt.savefig('eval_summary/waymo_occ_hist.pdf')
