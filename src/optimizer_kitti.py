...
class OptimizerKitti:
    def __init__(self, gpu, kitti_dataset, hpams, hpams_pose_refiner=None, hpams_pose_regressor=None, model_epoch=None,
                 opt_pose=False, ada_posecode=0, reg_iters=3, opt_multiview=False, pred_wlh=False,
                 save_postfix='_kitti', num_workers=0, shuffle=False, save_freq=100, vis=0, num_samples2eval=None):
        super().__init__()
        self.hpams = hpams
        ...
        self.vis = vis
        self.num_samples2eval = num_samples2eval
        self.save_postfix = save_postfix
        self.nusc2kitti_boxfac = BOX_FAC
        if 'pred_wlh' in hpams['net_hyperparams'].keys() and hpams['net_hyperparams']['pred_wlh'] > 0 and self.pred_wlh:
            self.nusc2kitti_boxfac = 1.0  # if to use pred_wlh, no need to adjust ratio for the refiner
        ...
        self.dataloader = DataLoader(self.kitti_dataset, batch_size=1, num_workers=num_workers, shuffle=shuffle, pin_memory=True)
        ...
        for ii, sample in enumerate(self.kitti_dataset.all_valid_samples):
    ...
    def optimize_objs(self):
        ...
        for batch_idx, batch_data in enumerate(self.dataloader):
            if self.num_samples2eval is not None and batch_idx >= self.num_samples2eval:
                print(f'save result at batch {batch_idx - 1}')
                self.save_opts(batch_idx - 1)
                break
            if self.num_samples2eval is not None:
                print(f'num obj: {batch_idx}/{self.num_samples2eval}')
            else:
            ...
            # convert obj pose from kitti to nusc then all the process afterward is same
    ...
    def optimize_objs_w_pose(self):
        ...
        for batch_idx, batch_data in enumerate(self.dataloader):
            if self.num_samples2eval is not None and batch_idx >= self.num_samples2eval:
                print(f'save result at batch {batch_idx - 1}')
                self.save_opts_w_pose(batch_idx - 1)
                break
            if self.num_samples2eval is not None:
                print(f'num obj: {batch_idx}/{self.num_samples2eval}')
            else:
            ...
            while self.nopts < self.hpams['optimize']['num_opts']:
                ...
                if self.vis == 2 or (self.vis == 1 and (self.nopts == 0 or self.nopts == (self.hpams['optimize']['num_opts'] - 1))):
                    ...
                    with torch.no_grad():
                        ...
                        g_c = np.array([0, 1, 0]).astype(np.float)
    ...
    def dl_pose_update(self,
        ...
        for iter in range(0, iters):
            src_pose_batch = self.fw_pose_one_step(
                ...
                K_batch,
                K_inv_batch,
                nusc2kitti_boxfac=self.nusc2kitti_boxfac
            )
            pose_per_iter.append(src_pose_batch.squeeze().cpu().numpy())
    ...
    def fw_pose_one_step(self,
                         ...
                         K_batch,
                         K_inv_batch,
                         nusc2kitti_boxfac=1.1
                         ):

        # TODO: change the hand-coded scale for auto fit to nuscanes
        src_uv_batch = view_points_batch(corners_of_box_batch(src_pose_batch, wlh_batch, scale=nusc2kitti_boxfac),
