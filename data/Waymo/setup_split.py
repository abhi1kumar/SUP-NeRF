...
kitti_org_train['velodyne'] = os.path.join(curr_folder, 'training_org','replace', 'velodyne')
...
kitti_tra['velodyne'] = os.path.join(curr_folder, split, 'velodyne')
...
kitti_org_val['velodyne'] = os.path.join(curr_folder, 'validation_org','replace', 'velodyne')
...
kitti_val['dep'] = os.path.join(curr_folder, split, 'depth')
kitti_val['velodyne'] = os.path.join(curr_folder, split, 'velodyne')
...
mkdir_if_missing(kitti_val['velodyne'])
...
for line in text_lines:
    ...
    if parsed is not None:
        ...
        org_dep_path = os.path.join(kitti_org_val['dep'].replace('replace', seg), id + '.npy')
        org_lidar_path = os.path.join(kitti_org_val['velodyne'].replace('replace', seg), id + '.bin')
        ...
        new_dep_path = os.path.join(kitti_val['dep'], str(new_id) + '.npy')
        new_lidar_path = os.path.join(kitti_val['velodyne'], str(new_id) + '.bin')

        # make_symlink_or_copy(org_image_path, new_image_path, MAKE_SYMLINK = False)
        # make_symlink_or_copy(org_dep_path, new_dep_path, MAKE_SYMLINK = False)
        make_symlink_or_copy(org_lidar_path, new_lidar_path, MAKE_SYMLINK = False)
        # make_symlink_or_copy(org_calib_path, new_calib_path, MAKE_SYMLINK = False)
        # make_symlink_or_copy(org_label_path, new_label_path, MAKE_SYMLINK = False)
